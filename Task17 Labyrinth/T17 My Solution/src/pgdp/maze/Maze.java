...
public class Maze {
	...
	private static class Field extends JPanel {
		...
		public void paint(Graphics g) {
			super.paint(g);
			if (maze[x][y] == WALL) {
				GradientPaint gradient = new GradientPaint(10, 50, Color.GRAY,
						getWidth(), 0, Color.DARK_GRAY);
				((Graphics2D) g).setPaint(gradient);
			} else {
				GradientPaint gradient = new GradientPaint(0, 50, Color.WHITE,
						getWidth(), 0, Color.GRAY);
				((Graphics2D) g).setPaint(gradient);
			}

			g.fillRect(p.getLocation().x, p.getLocation().y, getWidth() * 2,
					getHeight());
		...
		private void paintSymbol(Graphics g, Color c) {
			GradientPaint gradient = new GradientPaint(15, 0, c, getWidth(), 0,
					Color.LIGHT_GRAY);
			((Graphics2D) g).setPaint(gradient);
			((Graphics2D) g).setRenderingHint(RenderingHints.KEY_ANTIALIASING,
					RenderingHints.VALUE_ANTIALIAS_ON);
			g.fillOval((int) (getWidth() * 0.3), (int) (getHeight() * 0.3),
		...
		private void drawPeng(Graphics g) {
			...
			((Graphics2D) g).drawImage(peng, 0, 0, getWidth(), getHeight(), 0,
					0, peng.getWidth(null), peng.getHeight(null), null);
		}
	}
	...
	private static void setup(int[][] area) {
		...
		JPanel content = new JPanel(
				new GridLayout(area[0].length, area.length));
	...
	public static int[][] generateMaze(int width, int height, boolean penguins,
			boolean seeded, int seed) {
