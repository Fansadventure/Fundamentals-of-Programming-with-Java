...
public class MiniJava {
	...
	public static int[][] generateBoard() {
		...
		for (int length = 4; length >= 2; length--) {
			...
			for (int x = 0; x < board.length - (width - 1); x++)
				for (int y = 0; y < board.length - (height - 1); y++)
					if (isValid(board, x, y, length, length))
						positions.add(new int[] { x, y });

			int[] selected = positions.get(randomInt(0, positions.size() - 1));
			for (int i = selected[0]; i < selected[0] + width; i++)
				for (int j = selected[1]; j < selected[1] + height; j++)
					board[i][j] = 1;
		}
	...
	private static boolean isValid(int[][] board, int x, int y, int width,
			int height) {
	...
	public static void writeConsole(char output) {
		System.out.print(output);
	}

	public static void writeLineConsole(char output) {
		writeLineConsole(Character.toString(output));
	}
}
